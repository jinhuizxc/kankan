package com.ychong.baselib.utils.ftp;
...
/**
 * @ClassName: gomall-master-application
 * @Description: 文件上传下载
 * @Author: zhanglei
 * @Date: 2019-07-27
 * @Version v2.0
...
public class FtpFileHelper {
    ...
    public boolean uploadFile(InputStream input, String fileName, String filePath) {
        ...
        try {
            int reply;
            ftp.connect("47.98.147.72", 21);// 连接FTP服务器
            ...
            // 切换到上传目录
            if (!ftp.changeWorkingDirectory("/home/ftpuser/images" + File.separator
                    + filePath)) {
                // 如果目录不存在创建目录
                String[] dirs = filePath.split("/");
                String tempPath = "/home/ftpuser/images";
                for (String dir : dirs) {
    ...
    public FTPClient getConnectionFtp() {
        ...
        try {
            //连接FTP服务器
            ftp.connect("47.98.147.72", 21);
            //下面三行代码必须要，而且不能改变编码格式，否则不能正确下载中文文件
            ftp.setControlEncoding("GBK");
            FTPClientConfig conf = new FTPClientConfig(FTPClientConfig.SYST_NT);
            conf.setServerLanguageCode("zh");
            //登录ftp
            success = ftp.login("ftpuser", "ftpuser");
            if (!FTPReply.isPositiveCompletion(ftp.getReplyCode())) {
                ftp.disconnect();
               Log.e("","连接服务器失败");
            }
            Log.e("----->连接FTP服务器" , success+"");
        } catch (IOException e) {
            e.printStackTrace();
        }
        return ftp;
     ...
     * @return boolean
     */
    public boolean uploadFile(FTPClient ftp, String filePath, String fileName, InputStream inputStream) {
        ...
        try {
            ...
            String path = "/home/ftpuser/images" + "/" + filePath;
            ftp.changeWorkingDirectory(path);//转移到指定FTP服务器目录
            FTPFile[] fs = ftp.listFiles();//得到目录的相应文件列表
            fileName = FtpFileHelper.changeName(fileName, fs);
