package com.ychong.picandvideo.ui.main.common;
...
public class PreViewActivity extends AppCompatActivity {
    ...
    private void preViewPic(String path) {
        ...
        if (path.contains("http://") || path.contains("https://")) {
            //来自网络
            Glide.with(getApplicationContext()).load(path).placeholder(R.mipmap.kankan).override(1200, 1500).into(picIv);
        } else {
            //来自本地
            File file = new File(path);
            if (file.exists()) {
                Glide.with(getApplicationContext()).load(file).placeholder(R.mipmap.kankan).override(1200, 1500).into(picIv);
            } else {
