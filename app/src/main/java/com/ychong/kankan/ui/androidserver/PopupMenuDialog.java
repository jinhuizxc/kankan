...
public class PopupMenuDialog {
    ...
    public void eventBusCallBack(EventBusMessage message) {
        ...
        if (WIFI_CONNECT_CHANGE_EVENT.equals(msg)) {
            NetworkInfo.State state = (NetworkInfo.State) message.object;
    ...
    private void onWifiConnecting() {
        ...
        mTxtTitle.setTextColor(context.getResources().getColor(R.color.color_6495ED));
    ...
    private void onWifiConnected(String ipAddr) {
        ...
        mTxtAddress.setText(String.format(context.getString(R.string.http_address), ipAddr, 10086));
        mButtonSplitLine.setVisibility(View.GONE);
        mBtnWifiSettings.setVisibility(View.GONE);
    }
    ...
    private void onDialogDismiss(DialogInterface dialog) {
        if (mUnbinder != null) {
            mUnbinder.unbind();
            EventBus.getDefault().post(new EventBusMessage(POPUP_MENU_DIALOG_SHOW_DISMISS, MSG_DIALOG_DISMISS));
